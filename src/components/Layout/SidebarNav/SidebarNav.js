  ...
  return (
    <nav>
      <ul className="sidebar__primary">
        {groups.map(group => (
          <SidebarNavGroup key={group.type} isOpen={group.type === openNavGroup} primaryItem={group.primaryItem} secondaryItems={group.secondaryItems} navGroupOpened={() => navGroupOpened(group.type)} />
        ))}
      </ul>
    </nav>
  )
